...
(function($){
	...
	function RealComplete($input, options){
		...
		if (this.options.verbose) console.groupCollapsed('RealComplete > new instance of realComplete on', $input[0].name, options);
		this.$input = $input;
	...
	RealComplete.prototype.returnProperties = function(){
		...
		if (this.options.verbose) console.debug('RealComplete > returnProperties', props);
		return props;j
	}
	
	RealComplete.prototype.createDatalist = function(){
		if (this.options.verbose) console.groupCollapsed('RealComplete > createDatalist');
		var _this = this;
	...
	RealComplete.prototype.closeDatalist = function(){
		if (this.options.verbose) console.debug('RealComplete > closeDatalist');
		if (this.isOpen){
			this.isOpen = false;
			this.$datalist.hide();
		}
	...
	RealComplete.prototype.updateDatalist = function(){
		if (this.options.verbose) console.groupCollapsed('RealComplete > updateDatalist');
		var options = '', option;
	...
	RealComplete.prototype.updateDisplay = function(includePositionning){
		if (this.options.verbose) console.groupCollapsed('RealComplete > updateDisplay');
		
		if (0 === this.$datalist[0].children.length) this.closeDatalist(); 
		
		if (includePositionning){
			if (this.options.verbose) console.debug('RealComplete > updateDisplay > positionning');
	...
	RealComplete.prototype.attachHandlers = function(){
		if (this.options.verbose) console.groupCollapsed('RealComplete > attachHandlers');
		var _this = this;
		this.$input
				.on('keyup paste',function(e){
														if (!_this.filterKeyUp) {
															if (_this.options.verbose) console.debug('RealComplete > attachHandlers > $input > keyup paste >', e.which, $(this)[0].value);
				...
				.on({
					...
					'change':function(e){
											if (_this.options.verbose) console.debug('RealComplete > attachHandlers > $input > change');
											if (!_this.selectedOption) _this.$inputClone[0].value = _this.$input[0].value; 
					...
					'open.realcomplete':function(e){_this.openDatalist()},
					'close.realcomplete':function(e){_this.closeDatalist()},
					'update.realcomplete':function(e, options){_this.update(options)},
					'destroy.realcomplete':function(e){_this.destroy()}
				});
	...
	RealComplete.prototype.processReleasedKeyEvent = function(e){
		if (this.options.verbose) console.groupCollapsed('RealComplete > processReleasedKeyEvent >', e.which);
	...
	RealComplete.prototype.processPressedKeyEvent = function(e){
		if (this.options.verbose) console.groupCollapsed('RealComplete > processPressedKeyEvent >', e.which);
	...
	RealComplete.prototype.update = function(options){
		if (this.options.verbose) console.groupCollapsed('RealComplete > update', options);
		if (options){
			var _this = this;
			this.processOptions(options);
			if (_this.options.verbose) console.debug('RealComplete > update > waiting for running tasks ... ');
	...
	RealComplete.prototype.applyFilter = function(){
		if (this.options.verbose) console.groupCollapsed('RealComplete > applyFilter');
		this.$inputClone[0].value = '';
		if (0 !== this.dataCount) {
			...
			if ('' !== inputString ? true : '' !== this.previousInputString) { 
				...
				if (this.messages.$noResult) this.messages.$noResult.detach();
				if (this.options.verbose) console.debug('RealComplete > applyFilter > something to filter');
				if ('' === inputCleanString) {
			...
			if (this.options.verbose) console.debug('RealComplete > applyFilter > matchingDataCount =', this.matchingDataCount);
			if (0 === this.matchingDataCount) {
				if (this.options.verbose) console.debug('RealComplete > applyFilter > noResult =', this.messages.$noResult);
				this.$datalist.prepend(this.messages.$noResult || '');
				this.highlightOption();
			} else {
				if (this.options.verbose) console.debug('RealComplete > applyFilter > result =', this.messages.$noResult);
				if (this.messages.$noResult) this.messages.$noResult.detach();
				this.highlightOption(this.$datalist.find('.rc-option:visible').first());
			}
		} else {
	...
	RealComplete.prototype.highlightOption = function($option){
		if (this.$highlightedOption) this.$highlightedOption.removeClass('on');
		if ($option) this.$highlightedOption = $option.addClass('on');
	}
	...
	RealComplete.prototype.processOptions = function(options){
		...
		if ('object' !== typeof options) {
			if (this.options.verbose) console.error('RealComplete > processOptions > wrong options', options);
		} else {
	...
	RealComplete.prototype.processData = function(data){
		...
		if ('object' !== typeof data) {
			if (this.options.verbose) console.error('RealComplete > processData > wrong data', typeof data);
		} else {
			...
			this.updateDatalist();
			if (this.selectedOption) {
				if (!this.rawData[this.selectedOption]) {
					this.selectedOption = false;
				}
			}
		}
	...
	RealComplete.prototype.processMessages = function(messages){
		if (this.options.verbose) console.groupCollapsed('RealComplete > processMessages >',messages);
		...
		for (var m in messages){
			...
			if ('' === this.messages['$'+m][0].innerHTML) this.messages['$'+m] = false;
		}
		if (this.options.verbose) {
	...
	RealComplete.prototype.processAjaxLoad = function(options){
		...
		if (!options.url) {
			if (this.options.verbose) console.warn('RealComplete > processAjaxLoad > no url');
